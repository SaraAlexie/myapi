...
{
    count: 12
    next: localhost:4000/api/v1/animals?offset=5limit=5
    previous: null
    results: [indeholder første fem dyr, array]
    url: localhost:4000/api/v1/animals?offset=0limit=5
}

*/

//  http://localhost:4000/api/v1/animals?offset=5limit=5
router.get("/animals", async function (request, response, next) {
...
router.post("/animals", auth, function (request, response, next) {
    ...
    try {
        let animal = new Animal({
            type: request.fields.type, // vi kan bruge fields fordi vi bruger formidable
