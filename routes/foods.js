...
router.post("/foods", auth, function (request, response, next) {
    ...
    try {
        let food = new Food({ // vi kan bruge fields fordi vi bruger formidable
...
router.patch("/foods/:foodId", auth, async function (request, response, next) {
    let { brand, productname, price, weight, animal } = request.fields
    let updateObject = {}
    ...
    if (animal) updateObejct.animal = animal;
